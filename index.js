...
(function () {
    ...
    function buildSystemPrompt() {
        return 'Ты — точный литературный редактор. Ты выполняешь ровно одну задачу: заменяешь указанные слова на СЛОВА-СИНОНИМЫ, не меняя ничего больше.';
    }
    ...
    function buildUserPrompt(text, words) {
        return [
            ...
            '',
            'Строгие правила:',
            '1. Заменяй слово на ОДНО слово-синоним (в крайнем случае максимально короткий эквивалент). НЕ превращай слово в описательную фразу.',
            '   Пример правильно: «собственничество» → «ревность» / «эгоизм» / «властность».',
            '   Пример НЕПРАВИЛЬНО: «собственничество» → «чувство собственности».',
            '2. Каждый раз подбирай РАЗНЫЕ синонимы, не повторяйся между вызовами.',
            '3. Сохраняй грамматическую форму (падеж, число, род, время) исходного слова, чтобы предложение оставалось согласованным.',
            '4. НЕ изменяй никакие другие слова, пунктуацию, форматирование, разметку (*курсив*, "кавычки" и т.п.).',
            '5. Верни ТОЛЬКО итоговый текст целиком, без пояснений, без префиксов, без кавычек-обёрток.',
            '',
            'Текст:',
            text,
        ].join('\n');
    ...
    async function handleMessageEdited(messageId) {
        ...
        if (settings.enabled && settings.autoProcess) {
            // Пересчитываем на основе нового текста
            await processMessage(messageId, true);
        }
    }
